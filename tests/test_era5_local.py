...
def test_annual_heat_threshold_counts_are_transparent():
    daily_max, daily_min = _partial_daily_frames()
    out = annual_heat_indicators_from_daily(
        daily_max,
        daily_min,
        2025,
    ...
    assert set(out["quality_flag"]) == {"PARTIAL_TEST"}
    assert "risk_score" not in out.columns


def test_production_annual_metrics_reject_partial_year():
    daily_max, daily_min = _partial_daily_frames()
    with pytest.raises(ValueError, match="Incomplete daily-maximum coverage"):
        annual_heat_indicators_from_daily(daily_max, daily_min, 2025)
